...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/04/23 18:46:03 by hlouar            #+#    #+#             */
/*   Updated: 2016/04/23 18:46:05 by hlouar           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "includes/ls.h"

t_ascii				*sub_subloop_t(t_liste *ptr, t_ascii *ref_addr)
{
	struct stat		first;
	struct stat		second;
	t_data			tmp;
...
void				subloop_t(t_ascii *ref_addr, t_liste *ptr)
{
	while (ref_addr->i != ref_addr->nb_node - 1)
	{
		ref_addr = sub_subloop_t(ptr, ref_addr);
		ptr = ptr->next;
		ref_addr->i++;
	}
}
